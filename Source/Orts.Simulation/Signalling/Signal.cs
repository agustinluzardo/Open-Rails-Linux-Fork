...
#pragma warning restore CA1002 // Do not expose generic lists
        ...
        public Signal(int reference, Signal source)
        {
            ...
            lockedTrains = new List<(int Key, int Path)>(source.lockedTrains);
        ...
        public void RestoreTrains(List<Train> trains)
        {
            if (EnabledTrain != null)
            {
                ...
                if (Index == foundTrain?.NextSignalObjects[Direction.Forward]?.Index)
                {
                    EnabledTrain = foundTrain.RoutedForward;
                    foundTrain.NextSignalObjects[Direction.Forward] = this;
                }
                // check if this signal is next signal backward for this train
                else if (Index == foundTrain?.NextSignalObjects[Direction.Backward]?.Index)
                {
                    EnabledTrain = foundTrain.RoutedBackward;
                    foundTrain.NextSignalObjects[Direction.Backward] = this;
                }
                else
                {
        ...
        public async ValueTask<SignalSaveState> Snapshot()
        {
            return new SignalSaveState()
            {
                SignalIndex = Index,
                EnabledTrainNumber = EnabledTrain?.Train.Number ?? -1,
                SignalRoute = SignalRoute != null ? await SignalRoute.Snapshot().ConfigureAwait(false) : null,
                NextActiveSignals = new Collection<int>(Signalfound),
                TrainRouteIndex = TrainRouteIndex,
                HoldState = HoldState,
                JunctionsPassed = junctionsPassed?.Count > 0 ? new Collection<int>(junctionsPassed) : null,
                FullRoute = fullRoute,
                AllowPartialRoute = allowPartialRoute,
                PropagatedAhead = propagated,
                PropagatedPreviously = isPropagated,
                ForcePropagationOnApproachControl = forcePropOnApproachControl,
                SignalNumClearAheadActive = signalNumberNormalHeads,
                RequestedNumClearAhead = requestedNumClearAhead,
                ApproachControlCleared = approachControlCleared,
                ApproachControlSet = approachControlSet,
                ClaimLocked = claimLocked,
                OverridePermission = OverridePermission,
                LockedTrains = new Collection<(int, int)>(lockedTrains),
            };
        }
        ...
        public async ValueTask Restore(SignalSaveState saveState)
        {
            ArgumentNullException.ThrowIfNull(saveState, nameof(saveState));

            Index = saveState.SignalIndex;
            if (null != saveState.SignalRoute)
                await SignalRoute.Restore(saveState.SignalRoute).ConfigureAwait(false);
            Signalfound.Clear();
            Signalfound.AddRange(saveState.NextActiveSignals);
            TrainRouteIndex = saveState.TrainRouteIndex;
            HoldState = saveState.HoldState;

            junctionsPassed.Clear();
            if (saveState.JunctionsPassed != null)
            {
                junctionsPassed.AddRange(saveState.JunctionsPassed);
                foreach (int item in junctionsPassed)
                {
                    TrackCircuitSection.TrackCircuitList[item].SignalsPassingRoutes.Add(Index);
                }
            }

            // set dummy train, route direction index will be set later on restore of train
            if (saveState.EnabledTrainNumber >= 0)
            {
                Train train = new Train(saveState.EnabledTrainNumber);
                Train.TrainRouted routedTrain = new Train.TrainRouted(train, Direction.Forward);
                EnabledTrain = routedTrain;
            }
            else
                EnabledTrain = null;


            fullRoute = saveState.FullRoute;
            allowPartialRoute = saveState.AllowPartialRoute;
            propagated = saveState.PropagatedAhead;
            isPropagated = saveState.PropagatedPreviously;
            ForcePropagation = false; // preset (not stored)
            forcePropOnApproachControl = saveState.ForcePropagationOnApproachControl;
            SignalNumClearAheadActive = saveState.SignalNumClearAheadActive;
            requestedNumClearAhead = saveState.RequestedNumClearAhead;
            approachControlCleared = saveState.ApproachControlCleared;
            approachControlSet = saveState.ApproachControlSet;
            claimLocked = saveState.ClaimLocked;
            OverridePermission = saveState.OverridePermission;

            //  Retrieve lock table
            lockedTrains = saveState?.LockedTrains.ToList() ?? new List<(int, int)>();
        }
        ...
        public void SetSignalDefaultNextSignal()
        {
            ...
            while (trackCircuitSection != null && trackCircuitSection.CircuitType == TrackCircuitType.Normal)
            {
                ...
                // normal signal
                if (defaultNextSignal[SignalFunctionType.Normal] < 0)
                {
                    if (trackCircuitSection.EndSignals[direction] != null)
                    {
                        defaultNextSignal[SignalFunctionType.Normal] = trackCircuitSection.EndSignals[direction].Index;
                        completedFixedRoute = true;
                    }
                }

                // other signals
                for (int i = SignalFunctionType.Distance; i < signalEnvironment.OrtsSignalTypeCount; i++)
                {
                ...
                direction = trackCircuitSection.Pins[direction, SignalLocation.NearEnd].Direction;
                trackCircuitSection = TrackCircuitSection.TrackCircuitList[trackCircuitSection.Pins[currentDirection, SignalLocation.NearEnd].Link];
            }
        ...
        {
            return SignalHeads.Any(head => head.SignalFunction == SignalFunctionType.Normal);
        }
        ...
        /// </summary>
        public SignalAspectState SignalMR(SignalFunctionType signalType)
        {
        ...
        /// standard version without state return using MSTS type parameter
        /// </summary>
        public SignalAspectState SignalLR(SignalFunctionType signalType)
        {
        ...
        public SignalAspectState SignalLR(int signalType)
        {
            ...
            return sigAspSet ? sigAsp : signalType == SignalFunctionType.Normal ? SignalAspectState.Clear2 : SignalAspectState.Unknown;
        }//this_sig_lr
        ...
        /// </summary>
        public bool SignalNoSpeedReduction(SignalFunctionType signalType)
        {
            SignalAspectState sigAsp = SignalAspectState.Stop;
        ...
        /// </summary>
        internal SpeedInfo SpeedLimit(SignalFunctionType signalType)
        {
        ...
        {
            int nextSignal = Signalfound[SignalFunctionType.Normal];
            if (nextSignal < 0)
            {
                nextSignal = SONextSignal(SignalFunctionType.Normal);
                Signalfound[SignalFunctionType.Normal] = nextSignal;
            }
            if (nextSignal >= 0)
            {
                Signal nextSignalObject = signalEnvironment.Signals[nextSignal];
        ...
        public int SignalHasNormalSubtype(int subtype)
        {
            foreach (SignalHead head in SignalHeads)
            {
                if (head.SignalFunction == SignalFunctionType.Normal && head.NormalSubType.Index == subtype)
                {
                    return 1;
                }
            }
        ...
        public int Switchstand(int aspect1, int aspect2)
        {
            ...
            if (!nextSwitchIndex.HasValue)
            {
                ...
                while (!switchFound)
                {
                    ...
                    {
                        if (thisSection.Pins[pinIndex, SignalLocation.FarEnd].Link >= 0) // facing point
                        {
                            switchFound = true;
                    ...
                    sectionDirection = thisSection.Pins[pinIndex, SignalLocation.NearEnd].Direction;

                    if (thisSection.CircuitType != TrackCircuitType.EndOfTrack && thisSection.Pins[pinIndex, SignalLocation.NearEnd].Link >= 0)
                    {
                        thisSection = TrackCircuitSection.TrackCircuitList[thisSection.Pins[pinIndex, SignalLocation.NearEnd].Link];
                    }
                    else
                    {
                        break;
        ...
        public bool CheckRouteSet(int mainNode, int junctionNode)
        {
            ...
            if (retry || !SignalNormal() || MultiPlayerManager.IsMultiPlayer())
            {
                ...
                {
                    int sectionIndex;
                    if (section.ActivePins[direction, SignalLocation.NearEnd].Link >= 0)
                    {
                        newDirection = section.ActivePins[direction, SignalLocation.NearEnd].Direction;
                        sectionIndex = section.ActivePins[direction, SignalLocation.NearEnd].Link;
                    }
                    else
                    {
                        newDirection = section.ActivePins[direction, SignalLocation.FarEnd].Direction;
                        sectionIndex = section.ActivePins[direction, SignalLocation.FarEnd].Link;
                    }
                    ...
                    {
                        newDirection = section.Pins[direction, SignalLocation.NearEnd].Direction;
                        sectionIndex = section.Pins[direction, SignalLocation.NearEnd].Link;
                    }
        ...
        public int SONextSignal(int signalType)
        {
            ...
            if (reqtype == SignalFunctionType.Speed)
            {
                if (EnabledTrain != null)
                {
            ...
            // for normal signals
            else if (reqtype == SignalFunctionType.Normal)
            {
            ...
            else
            {
                ...
                if (sectionSet)
                {
                    trackCircuit = section.ActivePins[direction, SignalLocation.NearEnd].Link;
                    direction = section.ActivePins[direction, SignalLocation.NearEnd].Direction;
                }
            }
            ...
            while (sectionSet && trackCircuit > 0 && signalFound < 0)
            {
                ...
                if (signalFound < 0)
                {
                    ...
                    if (sectionSet)
                    {
                        trackCircuit = section.ActivePins[pinIndex, SignalLocation.NearEnd].Link;
                        direction = section.ActivePins[pinIndex, SignalLocation.NearEnd].Direction;
                        if (trackCircuit == -1)
                        {
                            trackCircuit = section.ActivePins[pinIndex, SignalLocation.FarEnd].Link;
                            direction = section.ActivePins[pinIndex, SignalLocation.FarEnd].Direction;
                        }
                    }
                }
            }
        ...
        {
            int routeListIndex = EnabledTrain.Train.ValidRoutes[Direction.Forward].GetRouteIndex(TrackCircuitIndex, EnabledTrain.Train.PresentPosition[Direction.Forward].RouteListIndex);
        ...
        public void StateUpdate()
        {
            ...
            foreach (SignalHead sigHead in SignalHeads)
            {
                if (sigHead.SignalFunction == SignalFunctionType.Normal)
                    sigHead.Update();
            }
        ...
        public TrackCircuitPartialPathRoute RequestClearSignalExplorer(TrackCircuitPartialPathRoute route, Train.TrainRouted train, bool propagated, int signalNumClearAhead)
        {
            ...
            {
                fullRoute = true;
                Signalfound[SignalFunctionType.Normal] = lastSection.EndSignals[lastDirection].Index;
            }
            ...
            bool extendRoute = ((SignalLR(SignalFunctionType.Normal) > SignalAspectState.Stop) || (internalBlockState <= InternalBlockstate.Reservable));
            ...
            if (extendRoute && fullRoute)
            {
                ...
                {
                    int nextSignalIndex = Signalfound[SignalFunctionType.Normal];
        ...
        public bool RequestClearSignal(TrackCircuitPartialPathRoute routePart, Train.TrainRouted train, int clearNextSignals, bool requestIsPropagated, Signal lastSignal)
        {
            ...
            if (EnabledTrain != null && EnabledTrain != train)
            {
                ...
                signalEnvironment.BreakDownRouteList(train.Train.ValidRoutes[train.Direction], routeListIndex, train);
                routeListIndex = otherTrain.Train.PresentPosition[otherTrain.Direction].RouteListIndex;
                signalEnvironment.BreakDownRouteList(otherTrain.Train.ValidRoutes[otherTrain.Direction], routeListIndex, otherTrain);
            ...
            Signalfound[SignalFunctionType.Normal] = nextSignal?.Index ?? -1;
            ...
            return SignalMR(SignalFunctionType.Normal) != SignalAspectState.Stop;
        }
...
#pragma warning disable CA1062 // Validate arguments of public methods
                    int forcedRouteSectionIndex = train.Train.ValidRoutes[Direction.Forward].GetRouteIndex(forcedTrackSection.Index, 0);
#pragma warning restore CA1062 // Validate arguments of public methods
            ...
            if (EnabledTrain != null && EnabledTrain.Train.NextSignalObjects[EnabledTrain.Direction] != null &&
                EnabledTrain.Train.NextSignalObjects[EnabledTrain.Direction].Index == Index)
            {
                propagated = false;
            }
            ...
            if (!signalHold)
            {
                ...
                else if (EnabledTrain != null && propagated)
                {
                    ...
                    if (firstSignal != null &&
                        firstSignal.Signalfound[SignalFunctionType.Normal] == Index &&
                        firstSignal.internalBlockState <= InternalBlockstate.Reservable &&
                        firstSignal.SignalLR(SignalFunctionType.Normal) == SignalAspectState.Stop)
                    {
                        GetPartialBlockState(route);
                    }
                }
            ...
            StateUpdate();
            SignalAspectState signalState = SignalLR(SignalFunctionType.Normal);
            ...
            if (EnabledTrain != null)
            {
                if (internalBlockState == InternalBlockstate.Reservable && !approachControlSet)
                {
                    ...
                    foreach (TrackCircuitRouteElement routelement in route)
                    {
                        ...
                        EnabledTrain.Train.EndAuthorities[EnabledTrain.Direction].LastReservedSection = routelement.TrackCircuitSection.Index;
                         ...
                         (internalBlockState != InternalBlockstate.Reserved && internalBlockState < InternalBlockstate.ReservedOther))
                {
                    ...
                    for (int i = 0; i < route.Count; i++)
                    {
                        ...
                        if (section.IsAvailable(EnabledTrain))
                        {
                            ...
                            EnabledTrain.Train.EndAuthorities[EnabledTrain.Direction].LastReservedSection = routeElement.TrackCircuitSection.Index;
                            lastSectionIndex = i;
                            lengthReserved += section.Length;
                        }
                        else
                ...
                // if claim allowed - reserve free sections and claim all other if first signal ahead of train
                         ...
                         EnabledTrain.Train.NextSignalObjects[Direction.Forward] != null && EnabledTrain.Train.NextSignalObjects[Direction.Forward].Index == Index)
                {
        ...
        private void PropagateRequest()
        {
            ...
            Signal nextSignal = null;
            if (Signalfound[SignalFunctionType.Normal] >= 0)
            {
                nextSignal = signalEnvironment.Signals[Signalfound[SignalFunctionType.Normal]];
            }

            TrackCircuitPartialPathRoute RoutePart;
            if (EnabledTrain != null)
            {
                RoutePart = EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction];   // if known which route to use
            }
            else
            {
            ...
            if (validBlockState && SignalLR(SignalFunctionType.Normal) == SignalAspectState.Stop && SignalNormal())
            {
            ...
            if (nextSignal != null && nextSignal.SignalLR(SignalFunctionType.Normal) >= SignalAspectState.Approach1 && nextSignal.FixedRoute && !nextSignal.Enabled && EnabledTrain != null)
            {
                ...
                if (firstSectionRouteIndex >= 0 && lastSectionRouteIndex >= 0)
                {
                    ...
                    int furtherSignalIndex = nextSignal.Signalfound[SignalFunctionType.Normal];
                    ...
                    while (furtherSignalIndex >= 0)
                    {
                        ...
                        if (furtherSignal.SignalLR(SignalFunctionType.Normal) >= SignalAspectState.Approach1 && !furtherSignal.Enabled && furtherSignal.FixedRoute)
                        {
                            ...
                            if (firstSectionRouteIndex >= 0 && lastSectionRouteIndex >= 0)
                            {
                                ...
                                furtherSignalIndex = furtherSignal.Signalfound[SignalFunctionType.Normal];
                            }
                            else
                            {
            ...
            else
            {
                ...
                while (section != null)
                {
                    ...
                    else
                    {
                        ...
                        TrackDirection pinIndex = direction;
                        int nextTC = section.ActivePins[pinIndex, SignalLocation.NearEnd].Link;
                        direction = section.ActivePins[pinIndex, SignalLocation.NearEnd].Direction;
                        if (nextTC == -1)
                        {
                            nextTC = section.ActivePins[pinIndex, SignalLocation.FarEnd].Link;
                            direction = section.ActivePins[pinIndex, SignalLocation.FarEnd].Direction;
                        }
        ...
        private bool GetPathBasedBlockState(TrackCircuitPartialPathRoute route, Train.TrainRouted train)
        {
            ...
            if (blockstate > InternalBlockstate.Reservable && train != null)
            {
                ...
                TrackCircuitPartialPathRoute trainRoute = train.Train.ValidRoutes[train.Direction];
                ...
                if (startAlternativeRoute > 0)
                {
                    ...
                    foreach (TrackCircuitRouteElement routeElement in altRoutePart)
                    {
                        ...
                        TrackDirection direction = routeElement.Direction;
                        newblockstate = section.GetSectionState(EnabledTrain, direction, newblockstate, route, Index);
        ...
        private bool GetLocationBasedBlockState(TrackCircuitPartialPathRoute route, Train.TrainRouted train, bool aiPermissionRequest)
        {
            ...
            if (altRouteAssigned && blockstate != InternalBlockstate.Reservable && blockstate != InternalBlockstate.Reserved)
            {
                ...
                {
                    int routeIndex = train.Train.ValidRoutes[Direction.Forward].GetRouteIndex(sectionNo, train.Train.PresentPosition[Direction.Forward].RouteListIndex);
                    train.Train.ValidRoutes[Direction.Forward][routeIndex].UsedAlternativePath = -1;
                }
        ...
        private void GetPartialBlockState(TrackCircuitPartialPathRoute route)
        {
            ...
            while (!endOfInfo)
            {
                ...
                if (!endOfInfo)
                {
                    thisSectionIndex = section.Pins[direction, SignalLocation.NearEnd].Link;
                    direction = section.Pins[direction, SignalLocation.NearEnd].Direction;
                }
            }
            ...
            if (blockstate <= InternalBlockstate.Reservable)
            {
                foreach (TrackCircuitRouteElement routeElement in additionalElements)
                {
                    TrackCircuitSection section = routeElement.TrackCircuitSection;
                    direction = routeElement.Direction;
                    blockstate = section.GetSectionState(EnabledTrain, direction, blockstate, additionalElements, Index);
                    if (blockstate > InternalBlockstate.Reservable)
                        break;           // break on first non-reservable section //
                }
            }
        ...
        public void ResetSignal(bool propagateReset)
        {
            ...
            if (propagateReset)
            {
                while (thisSignalIndex >= 0 && signalEnvironment.Signals[thisSignalIndex].EnabledTrain == thisTrain)
                {
                    ...
                    thisSignalIndex = thisSignal.Signalfound[SignalFunctionType.Normal];
                }
            }
            else
            {
        ...
        public void ResetRoute(int resetSectionIndex)
        {
            ...
            if (EnabledTrain != null && Signalfound[SignalFunctionType.Normal] < 0)
            {
                Signalfound[SignalFunctionType.Normal] = SONextSignalNormal(TrackCircuitNextIndex);
            }
        }
        ...
        public bool ApproachControlPosition(int requiredPositionM, bool forced)
        {
            ...
            if (!forced)
            {
                if (EnabledTrain.Train.NextSignalObjects[EnabledTrain.Direction] == null ||
                    EnabledTrain.Train.NextSignalObjects[EnabledTrain.Direction].Index != Index)
                {
            ...
            float distance = 0;
            TrackCircuitPartialPathRoute routePath = EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction];
            ...
            {
                float offset;
                if (EnabledTrain.Direction == Direction.Forward)
        ...
        public bool TrainHasCallOn(bool allowOnNonePlatform, bool allowAdvancedSignal)
        {
            ...
            // signal is not first signal for train
            Signal nextSignal = EnabledTrain.Train.NextSignalObjects[EnabledTrain.Direction];
        ...
        public bool RequiresNextSignal(int nextSignalId, int requiredPosition)
        {
            ...
            if (reqTrain.ValidRoutes == null || reqTrain.ValidRoutes[EnabledTrain.Direction] == null || reqTrain.ValidRoutes[EnabledTrain.Direction].Count <= 0)
            {
                return false;
            }
            ...
            if (reqTrain.PresentPosition[EnabledTrain.Direction].RouteListIndex < 0 ||
                reqTrain.PresentPosition[EnabledTrain.Direction].RouteListIndex >= reqTrain.ValidRoutes[Direction.Forward].Count)
            {
                return false;
            }
            ...
            int sectionIndex = reqTrain.ValidRoutes[EnabledTrain.Direction].GetRouteIndex(reqSection, reqTrain.PresentPosition[EnabledTrain.Direction].RouteListIndex);
        ...
        public int FindRequiredNormalSignal(int requiredValue)
        {
            ...
            if (EnabledTrain != null)
            {
                int startIndex = EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction].GetRouteIndex(TrackCircuitNextIndex, EnabledTrain.Train.PresentPosition[Direction.Forward].RouteListIndex);
                if (startIndex >= 0)
                {
                    for (int iRouteIndex = startIndex; iRouteIndex < EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction].Count; iRouteIndex++)
                    {
                        TrackCircuitRouteElement element = EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction][iRouteIndex];
        ...
        public SignalBlockState RouteClearedToSignal(int requiredSignalId, bool allowCallOn)
        {
            ...
            if (EnabledTrain?.Train.ValidRoutes[EnabledTrain.Direction] != null && requiredSignalId >= 0 && requiredSignalId < signalEnvironment.Signals.Count)
            {
                Signal otherSignal = signalEnvironment.Signals[requiredSignalId];

                TrackCircuitPartialPathRoute trainRoute = EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction];
        ...
        {
            lockedTrains.Add((trainNumber, subpath));
            return false;
        }

        public bool UnlockForTrain(int trainNumber, int subpath = 0)
        {
            return lockedTrains.Remove(lockedTrains.First(item => item.Key.Equals(trainNumber) && item.Path.Equals(subpath)));
        }

        public bool HasLockForTrain(int trainNumber, int subpath = 0)
        {
            return lockedTrains.Count > 0 && lockedTrains.Exists(item => item.Key.Equals(trainNumber) && item.Path.Equals(subpath));
        }
        ...
        public void DispatcherRequestHoldSignal(bool requestResetSignal)
        {
            ...
            SignalAspectState thisAspect = SignalLR(SignalFunctionType.Normal);
            ...
            {
                int signalRouteIndex = EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction].GetRouteIndex(TrackCircuitNextIndex, 0);
                if (signalRouteIndex >= 0)
                {
                    signalEnvironment.BreakDownRouteList(EnabledTrain.Train.ValidRoutes[EnabledTrain.Direction], signalRouteIndex, EnabledTrain);
                    ResetSignal(true);
                }
        ...
        {
            signalNumberNormalHeads = SignalHeads.Where(head => head.SignalFunction == SignalFunctionType.Normal).Count();
        }
        ...
        internal static void SetSignalCrossReference(TrackCircuitSection section)
        {
            ...
            foreach (TrackDirection direction in EnumExtension.GetValues<TrackDirection>())
            {
                ...
                if (signal != null)
                {
                    ...
                    signal.TrackCircuitNextIndex = section.Pins[direction, SignalLocation.NearEnd].Link;
                    signal.TrackCircuitNextDirection = section.Pins[direction, SignalLocation.NearEnd].Direction;
                }
            }
            ...
            foreach (TrackCircuitMilepost trackCircuitMilepost in section.CircuitItems.TrackCircuitMileposts)
            {
                ...
                {
                    trackCircuitMilepost.Milepost.SetCircuit(section.Index, trackCircuitMilepost.MilepostLocation[SignalLocation.NearEnd]);
                }
            }
        }
