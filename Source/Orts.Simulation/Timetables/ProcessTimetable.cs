    ...
    public class TimetableInfo
    {
        ...
        private TTTrain InitializePlayerTrain(TTTrainInfo reqTrain, ref Dictionary<string, AIPath> paths, ref List<TTTrain> trainList)
        {
            ...
            if (playerTrain.StartTime != playerTrain.ActivateTime)
            {
                ...
                playerTrain.FormedOfType = TimetableFormationCommand.Created;
            }
        ...
        private class TTTrainInfo
        {
                            ...
                            if (!string.IsNullOrEmpty(thisDisposeString))
                            {
                                ...
                                switch (disposeCommands.CommandToken)
                                {
                                    case string forms when forms.Equals("forms", StringComparison.OrdinalIgnoreCase):
                                        DisposeDetails = new DisposeInfo(DisposeInfo.DisposeType.Forms, disposeCommands, TimetableFormationCommand.TerminationFormed, TTTrain.Name);
                                        break;

                                    case string triggers when triggers.Equals("triggers", StringComparison.OrdinalIgnoreCase):
                                        DisposeDetails = new DisposeInfo(DisposeInfo.DisposeType.Triggers, disposeCommands, TimetableFormationCommand.TerminationTriggered, TTTrain.Name);
                                        break;

                                    case string staticTrain when staticTrain.Equals("static", StringComparison.OrdinalIgnoreCase):
                                        DisposeDetails = new DisposeInfo(DisposeInfo.DisposeType.Static, disposeCommands, TimetableFormationCommand.TerminationFormed, TTTrain.Name);
                                        break;

                                    case string stable when stable.Equals("stable", StringComparison.OrdinalIgnoreCase):
                                        DisposeDetails = new DisposeInfo(DisposeInfo.DisposeType.Stable, disposeCommands, TimetableFormationCommand.TerminationFormed, TTTrain.Name);
                                        break;

                                    case string pool when pool.Equals("pool", StringComparison.OrdinalIgnoreCase):
                                        DisposeDetails = new DisposeInfo(DisposeInfo.DisposeType.Pool, disposeCommands, TimetableFormationCommand.None, TTTrain.Name);
                                        break;
            ...
            public bool ProcessDisposeInfo(ref List<TTTrain> trainList, TTTrainInfo playerTrain, Simulator simulator)
            {
                ...
                TimetableFormationCommand formtype = TimetableFormationCommand.None;
                ...
                if (DisposeDetails.FormType == TimetableFormationCommand.TerminationFormed || DisposeDetails.FormType == TimetableFormationCommand.TerminationTriggered)
                {
                ...
                if (DisposeDetails.Stable && (trainFound || DisposeDetails.FormStatic))
                {
                    ...
                    else
                    {
                        ...
                        outTrain.FormedOfType = TimetableFormationCommand.TerminationFormed;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            formedTrain.FormedOfType = TimetableFormationCommand.TerminationFormed;

                            TrackCircuitPartialPathRoute lastSubpath = inTrain.TCRoute.TCRouteSubpaths[inTrain.TCRoute.TCRouteSubpaths.Count - 1];
                            if (inTrain.FormedOfType == TimetableFormationCommand.TerminationTriggered && formedTrain.Number != 0) // no need to set consist for player train
                            {
                ...
                if (formtype == TimetableFormationCommand.TerminationFormed && trainFound && DisposeDetails.RunRound)
                {
                    TTTrain usedTrain;
        ...
        private class DisposeInfo
        {
            ...
            public TimetableFormationCommand FormType;
            ...
            public DisposeInfo(DisposeType typeOfDispose, TTTrainCommands trainCommands, TimetableFormationCommand formType, string trainName)
            {
                FormTrain = false;
                FormStatic = false;
                Closeup = false;
                ...
                switch (typeOfDispose)
                {
                    ...
                    case DisposeType.Triggers:
                        ...
                        if (trainCommands.CommandQualifiers != null && (formType == TimetableFormationCommand.TerminationFormed || formType == TimetableFormationCommand.TerminationTriggered))
                        {
                    ...
                    case DisposeType.Static:
                        ...
                        FormStatic = true;
                        FormType = TimetableFormationCommand.None;
                        ...
                        foreach (TTTrainCommands.TTTrainComQualifiers stableQualifier in trainCommands.CommandQualifiers)
                        {
                            switch (stableQualifier.QualifierName)
                            {
                                ...
                                case "forms":
                                    ...
                                    FormStatic = false;
                                    FormType = TimetableFormationCommand.TerminationFormed;
                                    break;
                                ...
                                case "triggers":
                                    ...
                                    FormedTrain = stableQualifier.QualifierValues[0];
                                    FormStatic = false;
                                    FormType = TimetableFormationCommand.TerminationTriggered;
                                    break;
