...
namespace Orts.Formats.Msts.Parsers
{
    ...
    public class STFReader : IDisposable
    {
        ...
        internal (double factor, double offset) ParseUnitSuffix(ref string constant, Units validUnits)
        {
            ...
            if ((validUnits & Units.Temperature) > 0)
            {
                switch (suffix)
                {
                    ...
                    case "": return (1.0, 0);
                    case "degc": return (1, 0);
                    case "degf":  // Converting to equivalent degC
                        if (double.TryParse(constant, out double temperature))
                            constant = Temperature.Celsius.FromF(temperature).ToString();
                        return (1, 0);
                }
            }
        ...
        {
            uint hex = this.ReadHexBlock(STFReader.SwapColorBytes(defaultValue.GetValueOrDefault(Color.Black).PackedValue));
